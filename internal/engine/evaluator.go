...
// policy universe CI enforces.
func Evaluate(ctx context.Context, ix *Index, opts EvalOptions) (map[string]*EvalResult, map[string]bool, error) {
	if !ix.CleanCompile() {
	...
	modules, _, errs, err := loadModules(abs)
	if err != nil {
		return nil, nil, err
	}
	if len(errs) > 0 {
	...
	for _, path := range entrypoints {
		...
		for _, in := range inputs {
			...
			bodies := attributeBodies(buf, path)
			if len(bodies) > 0 {
				res.Attributed = true
...
func selectEntrypoints(ix *Index, opts EvalOptions) []string {
	...
	for _, r := range ix.Rules {
		...
		if opts.QueryPrefix != "" && !strings.HasPrefix(r.Path, opts.QueryPrefix) {
			continue
		}
...
func missingDataPaths(ix *Index, entrypoints []string, dataDoc map[string]any) []string {
	...
	for _, r := range ix.Rules {
		...
		for _, ref := range r.Refs {
			...
			seg := strings.SplitN(strings.TrimPrefix(s, "data."), ".", 2)[0]
			if _, ok := dataDoc[seg]; ok {
				continue
			}
			missing[s] = true
		}
