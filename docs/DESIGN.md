...
- Key quote it flagged: "the interface can be separate than the code… a backend service that runs headless" — the user is building a durable engine, not a UI.
...
Milestone 2 — **MCP server, the primary interface** (fast follower; no API key needed anywhere):
